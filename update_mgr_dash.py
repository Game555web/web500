...
new_content = """    function renderMGRDashboard() {
  ...
  <style>
    ...
    /* 1. Overview & Status */
    .overview-grid { display: grid; grid-template-columns: repeat(5, 1fr); gap: 15px; margin-bottom: 25px; }
    .status-card { background: #fff; padding: 20px; border-radius: 12px; border: 1px solid #e2e8f0; text-align: center; cursor: pointer; transition: all 0.2s; position: relative; overflow: hidden; }
    .status-card:hover { transform: translateY(-3px); box-shadow: 0 10px 15px -3px rgba(0,0,0,0.05); border-color: var(--gold); }
    .status-card h3 { font-size: 24px; font-weight: 800; margin: 10px 0 5px; color: var(--navy); }
    .status-card p { font-size: 12px; font-weight: bold; color: var(--gray-600); }
    .progress-bar-bg { background: #e2e8f0; height: 6px; border-radius: 3px; margin-top: 10px; overflow: hidden; }
    .progress-bar-fill { background: var(--gold); height: 100%; width: 80%; }
    ...
    /* 5. Tracking + Result */
    .tracking-bar { display: flex; justify-content: space-between; position: relative; padding: 20px 0; }
    .track-step { flex: 1; text-align: center; z-index: 2; position: relative; }
    .track-icon { width: 36px; height: 36px; background: #f8fafc; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 10px; font-size: 14px; font-weight: bold; color: var(--gray-400); border: 2px solid #e2e8f0; }
    .track-step.done .track-icon { background: #22c55e; color: #fff; border-color: #22c55e; }
    .track-step.active .track-icon { background: var(--gold); color: #fff; border-color: var(--gold); animation: pulse 2s infinite; }
    .track-line { position: absolute; top: 36px; left: 10%; right: 10%; height: 4px; background: #e2e8f0; z-index: 1; border-radius: 2px; }
    .track-line-fill { position: absolute; top: 0; left: 0; height: 100%; background: #22c55e; width: 35%; border-radius: 2px; }
  </style>
  ...
  <div class="mgr-container fade-in">
    ...
    <!-- 1. Overview & Status -->
    <h2 class="section-title">📊 1. Overview & Status (ภาพรวมโครงสร้างภารกิจ)</h2>
    <div class="overview-grid">
      <div class="status-card" style="border-bottom: 4px solid var(--navy);" onclick="showPage('org-structure')">
        <p>โครงสร้างทั้งหมด</p>
        <h3>24</h3>
        <div style="font-size:11px; color:#16a34a; font-weight:bold;">+2 ภารกิจใหม่</div>
      </div>
      <div class="status-card" style="border-bottom: 4px solid var(--gold);">
        <p>Draft (ร่างกำลังแก้)</p>
        <h3>3</h3>
        <div class="progress-bar-bg"><div class="progress-bar-fill" style="width:80%;"></div></div>
        <div style="font-size:11px; margin-top:5px; font-weight:bold; color:var(--gray-600);">พร้อมส่ง 80%</div>
      </div>
      <div class="status-card">
        <p>ส่ง HR อนุมัติ</p>
        <h3>5</h3>
        <div style="font-size:11px; color:var(--gray-500);">รอตรวจสอบเฉลี่ย 2 วัน</div>
      </div>
      <div class="status-card">
        <p>รออนุมัติสภาฯ</p>
        <h3>1</h3>
        <div style="font-size:11px; color:#d97706; font-weight:bold;">วาระการประชุม 15 พ.ค.</div>
      </div>
      <div class="status-card" style="border-bottom: 4px solid #22c55e;">
        <p>อนุมัติแล้ว (ปี 2567)</p>
        <h3>15</h3>
        <div style="font-size:11px; color:#16a34a; font-weight:bold;">ประกาศใช้อย่างเป็นทางการ</div>
      </div>
    </div>
    ...
    <div style="display:flex; justify-content:space-between; align-items:flex-end; margin-bottom:15px; margin-top:25px;">
      ...
      <button class="btn btn-outline" onclick="showPage('ปรับโครงสร้าง')">🛠️ เข้าโหมดปรับโครงสร้างเต็มรูปแบบ</button>
    </div>
    ...
    <!-- 5. Tracking + Result -->
    <h2 class="section-title">📍 5. Tracking + Result (ติดตามสถานะการอนุมัติ)</h2>
    <div class="panel" style="margin-bottom:40px; padding:30px;">
      <div class="tracking-bar">
        <div class="track-line"><div class="track-line-fill"></div></div>
        
        <div class="track-step done">
          <div class="track-icon">✓</div>
          <div style="font-size:13px; font-weight:800; color:var(--navy); margin-bottom:3px;">Draft & Submit</div>
          <div style="font-size:10px; color:var(--gray-500); background:#f1f5f9; padding:2px 6px; border-radius:4px; display:inline-block;">รศ.ดร.วีรชัย (1 พ.ค.)</div>
        </div>
        
        <div class="track-step active">
          <div class="track-icon">2</div>
          <div style="font-size:13px; font-weight:800; color:var(--gold); margin-bottom:3px;">HR Verification</div>
          <div style="font-size:10px; color:#92400e; background:#fef3c7; padding:2px 6px; border-radius:4px; display:inline-block;">กำลังตรวจสอบข้อมูล</div>
        </div>
        
        <div class="track-step">
          <div class="track-icon">3</div>
          <div style="font-size:13px; font-weight:800; color:var(--gray-500); margin-bottom:3px;">Council Approval</div>
          <div style="font-size:10px; color:var(--gray-400);">รอเข้าวาระการประชุมสภาฯ</div>
        </div>
        
        <div class="track-step">
          <div class="track-icon">4</div>
          <div style="font-size:13px; font-weight:800; color:var(--gray-500); margin-bottom:3px;">Official Release</div>
          <div style="font-size:10px; color:var(--gray-400);">ประกาศใช้อย่างเป็นทางการ</div>
        </div>
      </div>
      
      <div style="margin-top:40px; padding-top:25px; border-top:1px solid #e2e8f0; display:flex; justify-content:space-between; align-items:center;">
        <div style="display:flex; align-items:center; gap:15px;">
          ...
          <div>
            <div style="font-weight:800; font-size:14px; color:var(--navy);">ผลลัพธ์สุดท้าย (Final Approved Structure)</div>
            <div style="font-size:11px; color:var(--gray-500); margin-top:2px;">เอกสารรายงานและประกาศโครงสร้างจะสามารถดาวน์โหลดได้ที่นี่เมื่อผ่านการอนุมัติ</div>
          </div>
        </div>
        <div style="display:flex; gap:12px;">
          <button class="btn btn-outline" disabled style="opacity:0.5; cursor:not-allowed;">📥 Export PDF</button>
          <button class="btn btn-outline" disabled style="opacity:0.5; cursor:not-allowed;">🔗 ส่งมอบข้อมูลเชื่อมต่อมอดูลอื่น</button>
        </div>
      </div>
    </div>
